...
class Complaint(Base, TimestampMixin):
    ...
    sentiment: Mapped[str | None] = mapped_column(String(80), index=True, nullable=True)
    priority: Mapped[str | None] = mapped_column(String(80), index=True, nullable=True)
    confidence_score: Mapped[float | None] = mapped_column(Float, nullable=True)
    ai_explanation: Mapped[str | None] = mapped_column(Text, nullable=True)
    status: Mapped[str] = mapped_column(String(80), default="Pending Analysis", index=True, nullable=False)
    department: Mapped[str | None] = mapped_column(String(180), index=True, nullable=True)
