...
function DistributionMeter({ item, total }) {
  ...
  const color = item.label === 'Critical' || item.label === 'High' || item.label === 'Negative' ? 'bg-crimson-600' : item.label === 'Medium' ? 'bg-amber-500' : 'bg-emerald-500';
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-3xl text-zinc-400">Upload CSV or XLSX complaint files, run AI classification, and persist analyzed cases to PostgreSQL.</p>
        </div>
        ...
        <Card className="overflow-hidden">
          ...
          <CardBody>
            ...
            <motion.button
              type="button"
              onClick={pickFile}
              animate={selectedFile && !processing ? { boxShadow: ['0 0 0 rgba(220,38,38,0)', '0 0 34px rgba(220,38,38,0.18)', '0 0 0 rgba(220,38,38,0)'] } : undefined}
            ...
            >
              ...
              <span className="mt-5 font-display text-xl font-bold text-white">{selectedFile?.name || 'Drop or select enterprise complaint file'}</span>
              <span className="mt-2 max-w-md text-sm leading-6 text-zinc-500">Expected columns: {uploadTemplateColumns.join(', ')}.</span>
            </motion.button>
          ...
          <CardBody>
            <div className="mb-6">
              ...
              <div className="h-3 overflow-hidden rounded-full bg-zinc-900">
                <motion.div className="h-full bg-gradient-to-r from-crimson-900 via-crimson-600 to-red-400" animate={{ width: `${progress}%` }} transition={{ duration: 0.35 }} />
              </div>
              <p className="mt-3 text-sm text-zinc-500">
                Estimated rows scanned: <span className="font-semibold text-white">{processedRows.toLocaleString()}</span>
              </p>
            </div>
              ...
              {uploadProcessingSteps.map((step, index) => {
                ...
                return (
                  <MotionDiv
                    ...
                    className={`rounded-lg border p-4 transition ${active ? 'border-crimson-600/40 bg-crimson-600/10 shadow-crimson' : complete ? 'border-emerald-500/20 bg-emerald-500/5' : 'border-white/10 bg-black/20'}`}
                  >
      ...
      <Card>
        ...
        <CardBody>
          {results.length ? (
            ...
            <div className="rounded-lg border border-white/10 bg-black/25 p-8 text-center text-sm text-zinc-500">Run an upload or open a previous batch to review analyzed complaints.</div>
          )}
        </CardBody>
      </Card>
      ...
      >
        {loadingAnalysis ? (
          <div className="rounded-lg border border-white/10 bg-black/25 p-8 text-center text-sm text-zinc-500">Loading batch intelligence and complaint previews.</div>
        ) : selectedUpload ? (
          <div className="space-y-5">
            ...
            <div className="grid gap-4 lg:grid-cols-3">
              {[
                ...
                <section key={title} className="rounded-lg border border-white/10 bg-black/25 p-4">
                  <p className="label-caps text-zinc-500">{title}</p>
            ...
            <section className="rounded-lg border border-white/10 bg-black/25 p-4">
              <p className="label-caps text-zinc-500">Processing logs</p>
              <div className="mt-4 space-y-2">
                {logs.map((log, index) => (
                  <div key={`${log.message}-${index}`} className="rounded-lg border border-white/10 bg-zinc-950/60 px-3 py-2 text-sm text-zinc-300">
                    <span className="font-semibold text-crimson-300">{log.level}</span> - {log.message}
                  </div>
                ))}
              </div>
            </section>
