      ...
      <div className="flex min-h-screen items-center justify-center px-4 pb-6 pt-24 sm:px-6 lg:px-8">
        <AuthCard
          ...
          aside={
            <div className="space-y-4">
              ...
              >
                <p className="label-caps text-crimson-300">Setup Flow</p>
                <div className="mt-4 space-y-3">
                  {[
                    ...
                    <div key={title} className="flex gap-2.5">
                      <span className="grid h-7 w-7 shrink-0 place-items-center rounded-full border border-crimson-500/30 bg-crimson-600/10 text-xs font-bold text-crimson-200">
                        {index + 1}
                      </span>
                      <div>
        ...
        >
          <form className="space-y-4" onSubmit={submit}>
            <AuthInput label="Owner Full Name" icon={UserRound} value={ownerName} onChange={(event) => setOwnerName(event.target.value)} placeholder="Irfan Marwat" error={errors.ownerName} autoComplete="name" />
            <AuthInput label="Company Name" icon={Building2} value={companyName} onChange={(event) => setCompanyName(event.target.value)} placeholder="Nexus Bank" error={errors.companyName} autoComplete="organization" />
          ...
          <p className="mt-4 text-center text-sm text-zinc-500">
            ...
            <Link to="/admin/login" className="font-semibold text-crimson-300 transition hover:text-white">
              Login
            </Link>
          </p>
        </AuthCard>
